...
const DetailedTable = ({
  ...
  const [showDetailedMaterialsModal, setDetailedMaterialsShowModal] =
    useState(false);
  const [showDetailedRejOrdersModal, setDetailedRejOrdersShowModal] =
    useState(false);
    ...
    if (reportType === "detailed" && choice === 1) {
        ...
        <div>
          <div className="w-full flex flex-col mb-4">
            ...
            <p className="font-semibold">
              Created at: {currentDate} {currentTime}
            </p>
          </div>
                ...
                <table className="min-w-full bg-white border border-gray-300">
                  <thead>
                    <tr>
                      ...
                      <th className="border-b border-r px-4 py-2 text-left">
                        Recording Sales Person
                      </th>
                      <th className="border-b border-r px-4 py-2 text-right">
                        Quantity Ordered
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {groupedData[productName].map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="border-b border-r px-4 py-2 text-right">
                          {item.qty_ordered} units
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      return (
        <div className="w-[60%] mx-auto flex flex-col gap-3">
            ...
            <table className="min-w-full bg-white border border-gray-300">
              ...
              <tbody className="">
                {reportData.map((item, index) => (
                  <tr key={index}>
                    <td
                      className="border-b border-r px-4 py-2 cursor-pointer hover:bg-slate-200 transition ease duration-70"
                      onClick={() => handlesSalesDrillDown(item.product_name)}
                    >
                      {item.product_name}
                    </td>
                    <td className="border-b px-4 py-2 text-right">
                      {item.total_sales} units
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <DetailedSalesModal
            isVisible={showDetailedSalesModal}
            startDate={startDate}
            endDate={endDate}
            choice={salesDrillDown}
            onClose={closeDetailedSalesModal}
          />
        </div>
      );
      ...
      return (
        <div className="w-full">
          ...
          <h3 className="text-2xl font-bold mb-4 bg-slate-300 p-2 rounded-md">
            No Variations
          </h3>
                        ...
                        <tr key={index}>
                          ...
                          <td
                            className={`border-b px-4 py-2 text-right ${
                              item.transac_type === "Purchased"
                                ? "text-green-500"
                                : "text-red-500"
                            }`}
                          >
                            {item.amount} {item.metric_unit}
                          </td>
                        </tr>
                      ))}
                  </tbody>
        ...
        <div className="w-full flex flex-col gap-4">
          <div>
            ...
            <p className="text-xs italic mt-2">
              Select a material to view its detailed report
            </p>
          </div>
          {Object.keys(groupedData).map((materialName) => (
            <div key={materialName} className="flex flex-col mb-6">
              <h2
                className="w-full py-1 px-4 rounded-t-md text-center text-xl font-bold bg-slate-300 hover:bg-slate-400 cursor-pointer transition ease duration-70"
                onClick={() => handleMaterialDrillDown(materialName)}
              >
                {materialName}
              </h2>
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-300">
                  <thead>
                    <tr>
                      <th className="border-b border-r text-left px-4 py-2">
                        Transaction Type
                      </th>
                      <th className="border-b border-r text-left px-4 py-2 text-right">
                        Amount
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {groupedData[materialName].map((item, index) => (
                      <tr key={index}>
                        ...
                        <td
                          className={`border-b border-r px-4 py-2 text-right ${
                            item.grouped_transac_type === "Purchased"
                              ? "text-green-500"
                              : "text-red-500"
                          }`}
                        >
                          {item.total_amt} {item.metric_unit}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <DetailedMaterialsModal
                  isVisible={showDetailedMaterialsModal}
                  startDate={startDate}
                  endDate={endDate}
                  choice={materialDrillDown}
                  onClose={closeDetailedMaterials}
                />
              </div>
            </div>
          ))}
        ...
        <div className="w-full">
          <div className="w-full flex flex-col mb-4 gap-2">
            <h2 className="text-2xl font-bold">
              Detailed Rejected Sales Report
            </h2>
            <p className="font-semibold">
              Created at: {currentDate} {currentTime}
            </p>
                ...
                <tbody>
                  {reportData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="border-b px-4 py-2 text-right">
                        {item.insufficient_qty} {item.metric_unit}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      return (
        <div className="w-[80%] mx-auto flex flex-col gap-4">
          <div>
            <h2 className="text-2xl font-bold">
              Summarized Rejected Orders Report
            </h2>
            <p className="font-semibold">
              Created at: {currentDate} {currentTime}
            </p>
          ...
          <div className="flex flex-col mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-gray-300">
                <thead>
                  <tr>
                    <th className="border-b border-r text-left px-4 py-2 text-left">
                      Material Name
                    </th>
                    <th className="border-b border-r text-left px-4 py-2 text-right">
                      Total Insufficient Quantity
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {reportData.map((item, index) => (
                    <tr key={index}>
                      <td
                        className="border-b border-r px-4 py-2 text-left cursor-pointer hover:bg-slate-200 transition ease duration-70"
                        onClick={() => handleRejOrdersDrillDown(item.name)}
                      >
                        {item.name}
                      </td>
                      <td className="border-b border-r px-4 py-2 text-right">
                        {item.total_insufficient_qty} {item.metric_unit}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <DetailedRejOrdersModal
                isVisible={showDetailedRejOrdersModal}
                startDate={startDate}
                endDate={endDate}
                choice={rejDrillDown}
                onClose={closeDetailedRejOrders}
              />
            </div>
          </div>
          <h1 className="text-3xl font-bold mt-8 text-center">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-gray-300">
                <thead>
                  <tr>
                    <th className="border-b px-4 py-2 text-left">
                      Date of Manual Count
                    </th>
                    <th className="border-b px-4 py-2 text-left">
                      Material Name
                    </th>
                    <th className="border-b px-4 py-2 text-left">
                      Recording Stock Controller
                    </th>
                    <th className="border-b px-4 py-2 text-right">
                      Discrepancy Amount
                    </th>
                ...
                <tbody>
                  {reportData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="border-b px-4 py-2 text-right">
                        {item.qty_disc} {item.metric_unit}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="w-[80%] mx-auto flex flex-col gap-4">
          ...
          <div className="flex flex-col mb-6">
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {reportData.map((item, index) => (
                    <tr key={index}>
                      <td
                        className="border-b border-r px-4 py-2 text-left cursor-pointer hover:bg-slate-200 transition ease duration-70"
                        onClick={() => handleDiscDrillDown(item.name)}
                      >
                        {item.name}
                      </td>
                      <td className="border-b border-r px-4 py-2 text-right">
                        {item.total_disc} {item.metric_unit}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <DetailedDiscModal
                isVisible={showDetailedDiscModal}
                startDate={startDate}
                endDate={endDate}
                choice={discDrillDown}
                onClose={closeDetailedDiscrepancies}
              />
            </div>
          </div>
