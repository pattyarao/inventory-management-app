...
const PurchaseList = (props) => {
  ...
  const handleAddMaterials = (discardItem) => {
    ...
    discardItem.map((item) => {
      ...
      const isInList = isMaterial
        ? updatedDiscardedList.some((discarded) => discarded.id === item.id)
        : updatedDiscardedList.some(
            (discarded) => discarded.id === item.material_id,
          );
      ...
      if (isMaterial && isInList) {
        ...
        console.log(
          "case 1 insert succesfull: ",
          updatedDiscardItem,
          updatedDiscardedList,
        );
      } else if (!isMaterial && isInList) {
        console.log(
  ...
  const addVariant = (productIndex) => {
    ...
    const newVariant = {
      name: newPurchaseList[productIndex].id,
      amount: 0,
      finalAmount: 0,
      unit: newPurchaseList[productIndex].mainMetric,
      quantity: 1,
    };
  ...
  const handleVariantNameChange = (
    ...
    if (event.target.value === "") {
      newPurchaseList[productIndex].variants[variantIndex].name =
        event.target.value;
      newPurchaseList[productIndex].variants[variantIndex].unit =
        newPurchaseList[productIndex].mainMetric;
  ...
  const handleUnitChange = (productIndex, variantIndex, event) => {
    ...
    const selectedUnit = unitsList.find(
      (unit) =>
        unit.id === newPurchaseList[productIndex].variants[variantIndex].unit,
    );
    let ratio = selectedUnit.ratio;
    newPurchaseList[productIndex].variants[variantIndex].finalAmount =
      ratio * newPurchaseList[productIndex].variants[variantIndex].amount;
    setPurchaseList(newPurchaseList);
  };
  ...
  const handleRemove = (productIndex, variantIndex) => {
    ...
    if (
      newPurchaseList[productIndex].id ===
      newPurchaseList[productIndex].variants[variantIndex].name
    ) {
    ...
    >
                              ...
                              <>
                                ...
                                <div className="col-span-1 me-5">
                                  <div className="relative">
                                    ...
                                    >
                                      ...
                                      {variantsList.map(
                                        (variantItem, variantItemIndex) => (
                                          <>
                                            {variantItem.material_id ===
                                            material.id ? (
                                ...
                                <div className="mt-3 col-span-1 flex flex-row h-10 w-full rounded-lg relative bg-transparent mt-1">
                                  ...
                                  <input
                                    ...
                                    disabled={
                                      variant.name != material.id ? false : true
                                    }
                                  />
                                  {material.id != variant.name ? (
                                    <button
      ...
      {addVariantCondition ? (
        <AddNewVariant
          ...
          unit={unit}
          onClose={() => setAddVariantCondition(false)}
        />
      ) : null}
      ...
      {addUnitCondition ? (
        <AddNewUnit
          material_id={materialID}
          unit={unit}
          onClose={() => setAddUnitCondition(false)}
        />
      ) : null}
    </div>
  );
